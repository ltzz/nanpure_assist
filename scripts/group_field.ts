...
class FieldGroupManage {
  ...
  static switchBoard(bnum: number){
    FieldGroupManage.selectedBoard = bnum;
    ui.setCellsDim2( fields[FieldGroupManage.selectedBoard].board );
    $("#field" + bnum).css("border", "thin solid #ff6666");
  }
}

function miniBoardMouseDownEvent(evt: JQueryMouseEventObject) {
  ...
  const b_x = mousePoint.mx - evt_parent.offset().left;
  const b_y = mousePoint.my - evt_parent.offset().top;
...
function miniBoardMouseMoveEvent(evt: JQueryMouseEventObject) {
  var evt_parent_id = $(evt.target).parents("div")[0].id;
  var evt_parent = $("#" + evt_parent_id);
...
function miniBoardMouseUpEvent(evt: JQueryMouseEventObject) {
...
function GF_Init() {
  ...
  var addBoard = function() {
    ...
    $("<div>", {
      ...
      mousedown: miniBoardMouseDownEvent,
      mousemove: miniBoardMouseMoveEvent,
      mouseup:   miniBoardMouseUpEvent
...
function moveBoardJob(evt: JQueryMouseEventObject, isTouchEvent: boolean, boardId: number){
  const mousePoint = getPointFromMouseEvent(evt, isTouchEvent);
...
function getDragOffset( mousePoint: MousePoint, idx: number ){
  const mouseRPoint = getRelativePointFG( mousePoint );
